...
void t_py_generator::init_generator() {
  ...
  f_consts_ << py_autogen_comment() << endl << py_imports() << endl << (gen_relative_imports_ ? "from .ttypes import *" : "from ttypes import *")
            << endl << endl;
}
...
void t_py_generator::generate_service(t_service* tservice) {
  ...
  if (tservice->get_extends() != NULL) {
    ...
    if (gen_relative_imports_){
      f_service_ << "from " << get_real_py_module_rel(program, gen_twisted_) 
                 << " import " << tservice->get_extends()->get_name() 
                 << " as " << get_real_py_module_as(program, gen_twisted_) << "_" 
                 << tservice->get_extends()->get_name() << endl;
    } else {
      f_service_ << "import " 
                 << get_real_py_module(program, gen_twisted_) << "." 
                 << tservice->get_extends()->get_name() << endl;
    }
  }
...
void t_py_generator::generate_service_server(t_service* tservice) {
  ...
  if (gen_twisted_) {
    ...
    f_service_ << "class Processor(" << extends_processor << "Iface, " << (gen_async_ ? "TAsyncProcessor" : "TProcessor") << "):" << endl;
  }
  ...
  if (gen_tornado_) {
    ...
    indent(f_service_) << "def process(self, iprot, oprot, completed):" << endl;
    indent_up();
  } else {
  ...
  if (gen_twisted_ || gen_tornado_) {
    ...
    if (gen_async_) {
      f_service_ << indent() << "  self._processMap[name](self, seqid, iprot, oprot, completed)" << endl;
    }
    else {
